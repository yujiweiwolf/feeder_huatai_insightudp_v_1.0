//======================================================================================================
// htsc_client_interface.h
// htsc内部客户端接口定义头文件
// 版本：v1.0 20171129
// lujun
//======================================================================================================

#ifndef HTSC_MDC_GATEWAY_HTSC_CLIENT_INTERFACE_H
#define HTSC_MDC_GATEWAY_HTSC_CLIENT_INTERFACE_H
...
* gateway客户端接口类
*/
class LIB_EXPORT HtscClientInterface : public ClientInterface {
public:
	HtscClientInterface() {}
	virtual ~HtscClientInterface() {};
...
public:
	...
	*/
	virtual int LoginByServiceDiscovery(const std::string& ip, int port,
		...
		std::map<std::string, int>backup_list) = 0;
	/**
	...
	*/
	...
	//==================================================================================
	// 
	// 以下为htsc内部使用接口，可根据数据服务的ip、port、用户名、密码（或token）登录
	//
	//==================================================================================
	...
	* @param[in] port 端口
	*/
	virtual int AddServerInfo(const std::string& ip, int port) = 0;
	...
	* @return 0 成功 < 0 失败，错误信息见错误号
	*/
	virtual int LoginByToken(const std::string& user, const std::string& token) = 0;
